...
class OrderProcessor:
    ...
    def read_excel_file(self, filepath: str) -> Dict:
        ...
        try:
            ...
            # 读取基本信息(前4行)
            order_num = str(df.iloc[0, 0])
            part_num = str(df.iloc[1, 0])
            quantity = int(df.iloc[2, 0])
            piece_size = float(df.iloc[3, 0]) if len(df) > 3 and not pd.isna(df.iloc[3, 0]) else 0
            
            print(f"  订单号: {order_num}, 工件号: {part_num}, 数量: {quantity}, 单件大小: {piece_size}")
            ...
            for row_idx in range(4, len(df)):
                ...
                try:
                    ...
                    # ✅ 新增：读取热处理类型（第5列，索引4）
                    heat_treatment_type = None
                    # 检查是否有热处理机器
                    has_heat_treatment = any(m in [10, 11, 12] for m in available_machines)
                    
                    if has_heat_treatment:
                        # 读取第5列（索引4）作为热处理类型
                        if len(row) > 4 and not pd.isna(row[4]):
                            heat_treatment_type = str(row[4]).strip()
                            print(f"    工序{len(operations)+1}: 热处理类型={heat_treatment_type}")
                        else:
                            # 如果是热处理但没有指定类型，使用默认值
                            heat_treatment_type = "默认"
                            print(f"    ⚠️ 工序{len(operations)+1}: 热处理机器但未指定类型，使用默认类型")
                    ...
                    operation = {
                        ...
                        'heat_treatment_type': heat_treatment_type  # ✅ 新增字段
                    }
                    operations.append(operation)
                    
                    # 打印详细信息
                    type_info = f", 热处理类型={heat_treatment_type}" if heat_treatment_type else ""
                    print(f"    工序{len(operations)}: 调机={setup_time}, 固定={fixed_time}, "
                          f"单件={unit_time}, 可选机器={available_machines}{type_info}")

                except Exception as e:
                    continue
            
            # ✅ 这部分代码在 try 块内，与 for 循环对齐（4个空格）
            if not operations:
                print(f"  ❌ 没有读取到有效工序!")
                return None
        ...
        for filepath in sorted(excel_files):
            ...
            for batch_idx, batch_size in enumerate(batches):
                ...
                self.job_metadata[job_id] = {
                    ...
                    'piece_size': order.get('piece_size', 0),  # 新增
                    'total_batch_size': batch_size * order.get('piece_size', 0),  # 新增
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            for job_id in sorted(self.job_metadata.keys()):
                ...
                for op_idx, operation in enumerate(meta['operations']):
                    ...
                    # ✅ 获取热处理类型
                    heat_type = operation.get('heat_treatment_type', None)
                    heat_type_str = heat_type if heat_type else '-'
